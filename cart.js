    ...
    checkoutCompleteBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            cart.forEach((item, index) => {
                ...
                let fileName = item.img ? item.img.split('/').pop() : 'Desconhecido';
                if (!itemName) {
                    itemName = fileName;
                }

                message += `${index + 1}. Arte: ${itemName}\n`;
                if (item.img) message += `   Arquivo: ${fileName}\n`;
                message += `   Tamanho: ${sizeLabels[item.size] || item.size}\n`;
